...
# vals both temp until goose implemented
...
func _physics_process(delta):
	...
	velocity.x = (DASH_SPEED if facing_right else -DASH_SPEED) if dash.is_dashing() else move_spd * move_dir
...
#		print("is dash on cooldown: %s" % dash.is_on_cooldown())
		if !dash.is_on_cooldown():
			dash.start_dash(DASH_LENGTH, DASH_COOLDOWN)
			if curr_bird == $Duck:
				switch_to("Goose")
